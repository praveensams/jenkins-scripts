...
uploads()
    ...
    if [ $( grep $version /etc/preprod | wc -c ) -gt 4  ]
    then
       ...
       #)
        ...
        azcopy --source . --destination https://exprdstrgcdn.blob.core.windows.net/exprdeccdn01/ImagesSections/hybris/cdn/pre-prod/_ui/ --dest-key zqireCeb6SebWU1xS9uleijwSnE+Sf0JCwqrBIKx44DxEpAnDpp0yZdLenN6rojte/58/aS0gQ6toi/fL80NSg== --quiet --recursive   --resume "/mnt/myjournal"
       ls -l 
       )
...
azcopy --set-content-type 'text/css'  --source .  --destination  https://exprdstrgcdn.blob.core.windows.net/exprdeccdn01/ImagesSections/hybris/cdn/pre-prod/_ui/responsive/theme-extra/css --dest-key zqireCeb6SebWU1xS9uleijwSnE+Sf0JCwqrBIKx44DxEpAnDpp0yZdLenN6rojte/58/aS0gQ6toi/fL80NSg== --quiet   --resume "/mnt/myjournal" --recursive
       )
       echo $version >> /etc/preprod
    fi
    }
